...
JobFit GX is a local-first job/CV matching tool with a FastAPI backend, local embeddings, Ollama-powered analysis, a compact dashboard, and a Chromium Manifest V3 extension that feels like a browser-native job assistant.
...
```powershell
docker exec -it jobfit-gx-ollama-1 ollama pull qwen3:8b
```
...
4. Select `jobfit-gx/extension/dist`.
5. Pin the JobFit GX toolbar button if desired.

## Use With LinkedIn

1. Open a job page.
2. Click the JobFit GX extension icon or the floating `JF` button.
3. Paste the job text, or click **Extract visible page text**.
4. Select your saved CV.
5. Click **Analyze Job**.

JobFit GX does not scrape in the background. Extraction is explicit and limited to visible page text.
