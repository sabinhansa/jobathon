...
export async function getBaseUrl(): Promise<string> {
  const stored = await chrome.storage.local.get(["jobfitBaseUrl"]);
  return stored.jobfitBaseUrl || DEFAULT_BASE_URL;
}

export async function setBaseUrl(baseUrl: string): Promise<void> {
  await chrome.storage.local.set({ jobfitBaseUrl: baseUrl.replace(/\/$/, "") });
}
