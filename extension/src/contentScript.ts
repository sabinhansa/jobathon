...
function setupPanel(root: HTMLElement) {
  const drawer = root.querySelector<HTMLElement>("#jobathon-drawer")!;
  const fab = root.querySelector<HTMLButtonElement>("#jobathon-fab")!;
  const close = root.querySelector<HTMLButtonElement>("#jathon-close")!;
  const text = root.querySelector<HTMLTextAreaElement>("#jathon-text")!;
  const cv = root.querySelector<HTMLSelectElement>("#jathon-cv")!;
  const mode = root.querySelector<HTMLSelectElement>("#jathon-mode")!;
  const message = root.querySelector<HTMLElement>("#jathon-message")!;
  const resultBox = root.querySelector<HTMLElement>("#jathon-result")!;
  ...
  root.querySelector<HTMLButtonElement>("#jathon-extract")!.addEventListener("click", () => {
    text.value = document.body?.innerText?.slice(0, 60000) || "";
    setMessage("Visible page text extracted by explicit click.");
  });
  root.querySelector<HTMLButtonElement>("#jathon-open")!.addEventListener("click", () => {
    window.open("http://localhost:8000/app", "_blank", "noopener");
  });
  root.querySelector<HTMLButtonElement>("#jathon-analyze")!.addEventListener("click", async () => {
    ...
    try {
      ...
      root.querySelector<HTMLElement>("#jathon-score")!.textContent = String(latest.overall_score);
      root.querySelector<HTMLElement>("#jathon-summary")!.textContent = latest.summary;
  ...
  root.querySelectorAll<HTMLButtonElement>("[data-jathon-tab]").forEach((button) => {
...
#jobathon-root * { box-sizing: border-box; }
#jobathon-fab {
  position: fixed;
  right: 18px;
  bottom: 18px;
...
.jathon-head, .jathon-row, .jathon-score, .jathon-tabs {
  display: flex;
  align-items: center;
  gap: 8px;
}
